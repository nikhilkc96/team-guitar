...
if __name__ == "__main__":
    ...
    for i in range(0, len(inputs)):
        axs[i // 4, i % 4].scatter(list(sorted(z_pmds[i].keys())), list(x / n for x in z_pmds[i].values()))
        axs[i // 4, i % 4].set(xlabel='z')
        axs[i // 4, i % 4].set_title('P(z|u=' + str(inputs[i])+")")
        axs[i // 4, i % 4].axis([0., 128., 0., 0.02])
        if (i % 4 == 0):
    ...
    print("Sinse u is uniform, H(u) = log_(2)|M| = log_(2)|8| = 3")

    print("Mutual information I(u;z): ", mutual_information, "\n")
